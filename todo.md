...
## 10단계 — AI 위치 기반 음식점 추천 (Gemini API) *(기획 중 — 질문 답변 대기)*
- [ ] 위치 정보 수집 방식 확정
- [ ] 주변 음식점 검색에 쓸 데이터 소스 확정 (지도/장소 검색 API)
- [ ] Gemini API의 역할 범위 확정 (추천 문구 생성만 vs 후보 선별까지)
- [ ] 기존 등록 음식점과의 중복 판별 방식 확정
- [ ] 위 결정사항 바탕으로 세부 실행 계획 수립
